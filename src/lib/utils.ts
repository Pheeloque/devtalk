...
export const getCandidateInfo = (users: User[], candidateId: string) => {
  ...
  return {
    name: candidate?.name || "Неизвестный кандидат",
...
export const getInterviewerInfo = (users: User[], interviewerId: string) => {
  ...
  return {
    name: interviewer?.name || "Неизвестный собеседующий",
...
export const getMeetingStatus = (interview: Interview) => {
  ...
  if (interview.status === "completed" || interview.status === "failed" || interview.status === "succeeded")
    return "completed";
